...
def compressString(string):
    # Создаем пустую строку для сжатой строки
    compressedString = ""
    # Инициализируем счетчик символов
    count = 1
    # Проходим по всем символам строки, начиная со второго
    for i in range(1, len(string)):
        # Если текущий символ равен предыдущему, увеличиваем счетчик
        if string[i] == string[i - 1]:
            count += 1
        # Если текущий символ отличается от предыдущего, добавляем предыдущий символ в сжатую строку
        # и, если счетчик больше 1, добавляем количество повторений предыдущего символа
        else:
            compressedString += string[i - 1]
            if count > 1:
                compressedString += str(count)
            count = 1
    # Добавляем последний символ в сжатую строку
    compressedString += string[-1]
    # Если счетчик последнего символа больше 1, добавляем количество повторений последнего символа
    if count > 1:
        compressedString += str(count)
    # Возвращаем сжатую строку
    return compressedString

# Запрашиваем у пользователя строку для сжатия
